/**
 * Serpent - Encryption and decryption with Serpent
 *
...
object Serpent {
    ...
    private fun encrypt(plaintext: ByteArray, key: ByteArray, iv: ByteArray = ByteArray(0), mode: Mode): ByteArray {
        ...
        for (i in 0 until rounds) {
            ...
            System.arraycopy(result, 0, ciphertext, 16 * i, 16)
            if (mode == Mode.CBC) {
                initvector = result
            }
        }
    ...
    private fun decrypt(chipertext: ByteArray, key: ByteArray, iv: ByteArray = ByteArray(0), mode: Mode): ByteArray {
        ...
        for (i in 0 until rounds) {
            ...
            System.arraycopy(result, 0, plaintext, 16 * i, 16)
    ...
    private fun expandKey(key: ByteArray): IntArray {
        ...
        for (k in 0 until ROUNDS + 1) {
            ...
            sboxify(intArrayOf(w[4 * k], w[4 * k + 1], w[4 * k + 2], w[4 * k + 3]), state, (Serpent.ROUNDS + 3 - k) % 8)
            makeKey(w, state, k)
        }
    ...
    private fun sb0inv(state: IntArray) {
        ...
        val y0 =
            (state[0] xor state[2]) xor (((state[1] or state[2]) xor (state[3] or y2)) or (((state[0] or state[1]) xor (state[2] xor state[3])) and (y1 xor (state[0] or ((state[0] or state[1]) xor (state[2] xor state[3]))))))
    ...
    private fun checkKey(key: ByteArray) {
        if (key.size != 16 && key.size != 24 && key.size != 32) throw IllegalArgumentException("key size is not supported, must be 128, 192 or 256 bit")
    }
