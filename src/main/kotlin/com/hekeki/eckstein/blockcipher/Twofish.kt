...
object Twofish {
    ...
    private fun encrypt(plaintext: ByteArray, key: ByteArray, iv: ByteArray = ByteArray(0), mode: Mode): ByteArray {
        ...
        for (i in 0 until rounds) {
            ...
            System.arraycopy(result, 0, ciphertext, BLOCK_SIZE * i, BLOCK_SIZE)
            if (mode == Mode.CBC) {
                initvector = result
            }
        }
    ...
    private fun decrypt(chipertext: ByteArray, key: ByteArray, iv: ByteArray = ByteArray(0), mode: Mode): ByteArray {
        ...
        for (i in 0 until rounds) {
            ...
            System.arraycopy(result, 0, plaintext, BLOCK_SIZE * i, 16)
    ...
    internal fun checkKey(key: ByteArray) {
        if (key.size !in 8..32 || key.size % 8 != 0) throw IllegalArgumentException("key size is not supported")
    }
