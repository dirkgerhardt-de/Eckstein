...
class TwofishTest {
    ...
    fun `ECB with PKCS7 padding matches roundtrip`() {
        ...
        val plain = "Not block aligned data!!".toByteArray() // not multiple of 16

        val actual = Twofish.encryptECB(plain, key, Padding.SCHEME.PKCS7)
        val actualPlain = Twofish.decryptECB(actual, key, Padding.SCHEME.PKCS7)
    ...
    fun `CBC roundtrip works with real UTF-8 text`() {
        ...
        val encrypted = Twofish.encryptCBC(text.toByteArray(Charsets.UTF_8), key, iv, Padding.SCHEME.PKCS7)
    ...
    fun `invalid key size throws`() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            Twofish.encryptECB(plain, ByteArray(20))
        }
    }
    ...
    fun `non block aligned plaintext without padding throws`() {
        val key = Utils.randomBytes(16)
        assertThrows(IllegalArgumentException::class.java) {
            Twofish.encryptECB(ByteArray(15), key)
        }
    }
    ...
    fun `wrong iv size throws`() {
        ...
        val plain = ByteArray(16)
        assertThrows(IllegalArgumentException::class.java) {
            Twofish.encryptCBC(plain, key, ByteArray(8))
        }
    }
}
